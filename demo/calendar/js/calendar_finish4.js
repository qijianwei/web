window.onload=function(){
				...
				var upclickcount=1;
				var downclickcount=1;
				var Y=getTime().iYear;
				var M=nowMonth;
				var notearr=[];
				var oTagicon=document.querySelector(".tagicon");
//				var intoNum=0;
				//初始化
...
//					console.log()
				...
				//主面板中天气图案根据时间是白天还是晚上来更换
				var timerB;
				clearInterval(timerB);
				timerB=setInterval(function(){
					if (getTime().iHour>=6&&getTime().iHour<18) {
						oBgimg.src='img/dw.png';
					} else{
						oBgimg.src='img/nw.png';
					}
				},500);
				...
				oTags.forEach(function(ele){
					ele.onmouseover=function(ev){
						var ev=ev||window.event;
					
						ev.preventDefault(); 
						if (ele.offsetWidth-(ev.clientX-ele.offsetLeft)<4&&ele.offsetHeight-(ev.clientY-ele.offsetTop)<4) {
							ele.style.cursor='se-resize';
							ele.onmousedown=function(ev){
								var ev=ev||window.event;
								ele.mouseX=ev.clientX;
								ele.mouseY=ev.clientY;
								...
								document.onmousemove=function(ev){
									...
									if (ele.offsetWidth>=150) {
										ele.style.width=ele.W+ev.clientX-ele.mouseX+'px';
									}
									if (ele.offsetHeight>=150) {
										ele.style.height=ele.H+ev.clientY-ele.mouseY+'px';
									}
									document.onmouseup=function(){
										document.onmousemove=ele.onmousedown=null;
										ev.preventDefault(); 
									}
								}
						...
						}else if(ev.clientX>=ele.offsetLeft&&ev.clientX<=ele.offsetLeft+ele.offsetWidth&&ev.clientY>ele.offsetTop&&ev.clientY<ele.offsetTop+10){
							...
							ele.onmousedown=function(ev){
								...
								document.onmousemove=function(ev){
									var ev=ev||window.event;
									ele.style.left=ev.clientX-ele.mouseX+ele.L+'px';
									ele.style.top=ev.clientY-ele.mouseY+ele.T+'px';
									ev.preventDefault(); 
									document.onmouseup=function(){
										document.onmousemove=ele.onmousedown=null;
										ev.preventDefault(); 
...
//				document.onclick=function(){	
				...
				/*-----------------点击回到今日----------------*/
				oDateDetail.num=0;
				oDateDetail.onclick=function(){
//					YearandMonthclicknum=-1;
					oYearandMonth.innerHTML=this.innerHTML.substr(0,8);
							...
							animation(oCal_content,{
								...
								oCal_content.style.zIndex=999;
							});
							
							oUl2.innerHTML='';//清空ul2的所有li，因为我下面要重新设置li和日期
							
							showDays(getTime().iYear,getTime().iMonth-1);//显示点击后的这个月份的日期，在这里会加载所有li
						
							showClickDate(getTime().iYear,getTime().iMonth-1);//显示点击后的日期
...
//							moveMouse();
							...
							var nowY=Number(getTime().iYear);//当前年份，用于计数
				nowMonth=Number(getTime().iMonth);//真是的月份，3月就是3，用于计数
				selectedNum=-1;//记录点击的li的下标
				noteDate={
					iYear:getTime().iYear,
					iMonth:getTime().iMonth,//2月这里就是2
					iDate:getTime().iDate
				...
				M=nowMonth;
							...
							nowMonth=getTime().iMonth;
							Y=getTime().iYear;
							M=nowMonth;
							YearandMonthclicknum=-1;
							oDateDetail.num=1;
//							console.log(YearandMonthclicknum);
				}
				...
				oYearandMonth.onclick=function(){
					...
					if (YearandMonthclicknum==0/*&&oYearandMonthonOff*/) {//月份出现
						
//						animation(oSelect_Month,{
//							opacity:1
//						},200,'linear',function(){
//							oSelect_Month.style.zIndex=9999;
//							//设置每个li里面的年份
//							aS_li.forEach(function(ele4,index4){
//								ele4.innerHTML=index4+1+'月';
//								if(Number(ele4.innerHTML)==getTime().iMonth-1){
...
//						oYearandMonthonOff=false;
						...
						animation(oYearandMonth,{
							...
							if(oDateDetail.num==1){
								oYearandMonth.innerHTML=oYearandMonth.innerHTML.substr(0,4)+'年';
					...
					}else if(YearandMonthclicknum==1){//年份出现
//						console.log(YearandMonthclicknum,'111');
//						animation(oSelect_Month,{
//							opacity:1
//						},200,'linear',function(){
//							oSelect_Month.style.zIndex=9999;
...
//				里面的渲染页面可以封装成函数
				...
				function prev(){
					oPrev.onclick=function(){
						...
						if(YearandMonthclicknum==-1){
//							console.log(intoNum);
//							if (intoNum==1) {//说明是点击年月进入的
//								var Y=oYearandMonth.innerHTML.substr(0,4);
//								nowMonth=oYearandMonth.innerHTML.substr(5,2)+1;
//								
//								intoNum=0;
//							} else{
//								var Y=getTime().iYear;
...
//							}
							...
							oUl2.innerHTML='';//清空ul2的所有li，因为我下面要重新设置li和日期
//							M--;//每次点击，当前月份减1
							showDays(Y,M-1);//显示点击后的这个月份的日期，在这里会加载所有li
							
							showClickDate(Y,M-1);//显示点击后的日期
							
							//获取当月第一天的星期,这个变量是给右键菜单使用的
							nowFirstDayWeek=new Date(Number(oYearandMonth.innerHTML.substring(0,4)),Number(oYearandMonth.innerHTML.substring(5,7)-1),1,1,1,1).getDay();
							rightmune();
							moveMouse();
						}else if(YearandMonthclicknum==0){
							return;
						}else if(YearandMonthclicknum==1){
...
//							intoNum=1;
							animation(oSelect_Month,{
									...
									//设置每个li里面的年份
									aS_li.forEach(function(ele,index){
										ele.innerHTML=getTime().iYear-Number(String(getTime().iYear).substring(3))+index-12*upclickcount;
										if(Number(ele.innerHTML)==getTime().iYear){
											ele.style.borderColor='#0078d7';
											ele.style.background='#0078d7';
										}
								...
								animation(oYearandMonth,{
									...
									oYearandMonth.innerHTML=(getTime().iYear-Number(String(getTime().iYear).substring(3))-12*upclickcount)+'-'+(getTime().iYear-Number(String(getTime().iYear).substring(3))+11-12*upclickcount);               
									animation(oYearandMonth,{
										opacity:1
									},500,'linear');
								})
				...
				function oNext1(){
					oNext.onclick=function(){
						console.log(YearandMonthclicknum+'a')
						if (YearandMonthclicknum==-1) {
							oUl2.innerHTML='';//清空ul2的所有li，因为我下面要重新设置li和日期
...
//						alert(2);
						...
						addLi(noteTitle,noteTime,textdetail);
						...
						for (var i=0;i<oList_ul.children.length;i++) {
							...
							oList_ul.children[i].onclick=function(){
								...
								console.log('mm');
								//输入的内容还原
								oEventText1.value=this.children[1].innerHTML;
								oAlertText1.value=this.children[0].innerHTML;
								oEventDetail[1].value=this.children[2].children[0].innerHTML;
...
//					}
				...
				oRightmune.onclick=function(){
					...
					oAlertText.value=noteDate.iYear+"-"+noteDate.iMonth+"-"+toTwo(noteDate.iDate)+"T"+getTime().iHour+":"+getTime().iMinute;
				}
...
//				var oList_ul=document.querySelector(".list_ul");
					...
					for (var l=0;l<oList_ul.children.length;l++) {
						...
						oList_ul.children[l].oncontextmenu=function(ev){
							...
							oRightmunel1.onclick=function(){
								...
								notearr.splice(rightclicknum,1);
								console.log(notearr);
								deleteli();
							}
				...
				//新增li添加到ul 
				function addLi(addtitle,addtime,addcontent){
					oList_ul.innerHTML+="<li><h3>"+addtime+"</h3><h3>"+addtitle+"<h3><div class='content_note'>"+addcontent+"</div></li>"
					notearr.push({
						time:addtime,
						title:addtitle,
						content:addcontent
				...
				function rightmune(){
					...
					for (var i=nowFirstDayWeek;i<=getCountDays(Number(oYearandMonth.innerHTML.substring(0,4)),Number(oYearandMonth.innerHTML.substring(5,7)-1))+nowFirstDayWeek-1;i++) {
						...
						oUl2.children[i].oncontextmenu=function(ev){
							var firstDay = new Date(Number(oYearandMonth.innerHTML.substr(0,4)),Number(oYearandMonth.innerHTML.substr(5,2)-1),1).getDay();
							if (firstDay==0) {
								firstDay=7;
							}
							var ev=ev||event;
